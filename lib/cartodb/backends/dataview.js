...
DataviewBackend.prototype.getDataview = function (mapConfigProvider, user, params, callback) {
    ...
    step(
        ...
        function runDataviewQuery(err, node) {
            ...
            var layer = _.find(mapConfig.obj().layers, function(l){ return l.options.source && (l.options.source.id === sourceId); })
            ...
            if ( queryRewriteData ) {
                if ( node.type == 'source' ) {
                    ...
                    queryRewriteData = _.extend({}, queryRewriteData, { filters: filters, unfiltered_query: unfiltered_query });
                    var rewritten_query = overviewsQueryRewriter.query(unfiltered_query, queryRewriteData, { bounding_box: params.bbox });
