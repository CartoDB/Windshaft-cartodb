...
const getTokenFromContext = require('./lib/get_named_map_token_from_context.js');
...
module.exports = ({ allowNamedMap = false, useDefaultApiKey = false }) =>
    function getAuthorizationTokenMiddleware(req, res, next) {
        ...
        if (context.token) {
            const namedMapToken = getTokenFromContext(context);
            if (namedMapToken) {
                namedMapTokenExtractedFromRequest = setAuthContextWithNamedMap(context, { namedMapToken });
            }
        }
