...
MapController.prototype.createLayergroupFromTemplate = function(req, res, next) {
    const { user, mapconfig, rendererParams } = res.locals;
    const mapconfigProvider = new CreateLayergroupMapConfigProvider(mapconfig, user, this.userLimitsApi, rendererParams);
...
MapController.prototype.instantiateTemplate = function(req, res, next) {
    ...
    step(
        function afterLayergroupCreate() {
            const afterLayergroupCreate = self.afterLayergroupCreateBuilder({
