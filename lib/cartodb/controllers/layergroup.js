...
function LayergroupController(prepareContext, pgConnection, mapStore, tileBackend, previewBackend, attributesBackend,
                        surrogateKeysCache, userLimitsApi, layergroupAffectedTables, analysisBackend, metadataBackend,
                        templateMaps) {
...
LayergroupController.prototype.register = function(app) {
    app.get(
        ...
        authMiddleware({metadataBackend: this.metadataBackend, mapStore: this.mapStore, templateMaps: this.templateMaps}),
...
function createOptionsForMapStoreMapConfigProvider(context, fullPrivileges = false) {
    const options = {};
    ...
    options.signer = options.signer;
    options.token = context.token;
    options.interactivity = context.interactivity;
    ...
    options.own_filter = context.own_filter;
    options.filters = context.filters;
    options.own_filter = context.own_filter;
    options.no_filters = context.no_filters;
    options.bbox = context.bbox;
    options.start = context.start;
    options.end = context.end;
    options.column_type = context.column_type;
    options.bins = context.bins;
    options.aggregation = context.aggregation;
    options.offset = context.offset;
    options.q = context.q;
    options.categories = context.categories;
    options.context = context.context;
    options.fid = context.fid;

    options.dataviewName = context.dataviewName;
...
LayergroupController.prototype.dataview = function(req, res, next) {
    ...
    step(
        function retrieveDataview() {
            const options = createOptionsForMapStoreMapConfigProvider(res.locals, true);
...
LayergroupController.prototype.dataviewSearch = function(req, res, next) {
    ...
    step(
        function searchDataview() {
            ...
            self.dataviewBackend.search(mapConfigProvider, res.locals.user, req.params.dataviewName, options, this);
        },
...
LayergroupController.prototype.attributes = function(req, res, next) {
    ...
    step(
        function retrieveFeatureAttributes() {
            ...
            self.attributesBackend.getFeatureAttributes(mapConfigProvider, options, false, this);
        },
...
LayergroupController.prototype.getAffectedTables = function ({ dbConfig, layergroupId }, callback) {
    ...
    step(
        ...
        function findAffectedTables(err, sql) {
            ...
            const connection = self.pgConnection.getMasterConnection(dbConfig);
