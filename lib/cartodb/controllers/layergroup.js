...
LayergroupController.prototype.register = function(app) {
    ...
    app.get(app.base_url_mapconfig +
        '/:token/:layer/:z/:x/:y@:scale_factor?x.:format', cors(), userMiddleware,
    ...
    app.get(app.base_url_mapconfig +
        '/static/center/:token/:z/:lat/:lng/:width/:height@:scale_factor?x.:format', cors(), userMiddleware,
    ...
    app.get(app.base_url_mapconfig +
        '/static/bbox/:token/:west,:south,:east,:north/:width/:height@:scale_factor?x.:format', cors(), userMiddleware,
        this.bbox.bind(this));
...
LayergroupController.prototype.staticMap = function(req, res, width, height, zoom /* bounds */, center) {
    ...
    var options = {
        scaleFactor: Number.isFinite(+req.params.scale_factor) ? +req.params.scale_factor : 1
    };
    ...
    step(
        ...
        function getImage(err) {
            assert.ifError(err);
            if (center) {
                self.previewBackend.getImageByZoomAndCenter(
                    new MapStoreMapConfigProvider(self.mapStore, req.context.user, self.userLimitsApi, req.params),
                    format, width, height, zoom, center, options, this);
            } else {
                self.previewBackend.getImageByBounds(
                    new MapStoreMapConfigProvider(self.mapStore, req.context.user, self.userLimitsApi, req.params),
                    format, width, height, zoom /* bounds */, options, this);
            }
        },
