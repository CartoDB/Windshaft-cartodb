...
ReplaceSourcesMapConfigAdapter.prototype.getMapConfig = function(user, requestMapConfig, params, context, callback) {
    ...
    requestMapConfig.layers = requestMapConfig.layers.map(function(layer, layerIndex) {
        ...
        if (layerSourceId) {
            ...
            if (datasource) {
                layer.options.sql = datasource.getLayerQuery();
                ...
                layer.options.meta = datasource.getMetadata();
            } else {
    ...
    Object.keys(dataviews).forEach(function(dataviewName) {
        ...
        dataview.source.type = datasource.getType();
        dataview.source.meta = datasource.getMetadata();

        var ownFilterOff = {};
        ownFilterOff[dataviewName] = false;
        dataview.sql = {
            own_filter_on: datasource.getDataviewQuery(),
            own_filter_off: datasource.getDataviewQuery(ownFilterOff),
            no_filters: datasource.getDataviewQuery(false)
        };
    });
