...
var TTDatasource = require('../models/datasource/tt');
...
DataviewBackend.prototype.getDataview = function (mapConfigProvider, user, params, callback) {
    ...
    step(
        ...
        function runDataviewQuery(err, mapConfig) {
            ...
            var queryRewriteData = getQueryRewriteData(layer, dataviewDefinition, params);
            ...
            if (isTTDataview(layer, dataview)) {
                var dataviewQuery = TTDatasource.getDataviewQuery(
                    layer, dataview, dataviewName, bboxFilter, overrideParams, ownFilter
                );
                if (dataviewQuery !== null) {
                    return pg.query(dataviewQuery, function(err, resultSet) {
                        if (err) {
                            return callback(err);
                        }
                        return callback(err, resultSet.rows[0].dataview);
                    });
                }
            }
...
function getQueryRewriteData(layer, dataviewDefinition, params) {
...
function isTTDataview(layer) {
    return layer && layer.options.meta && layer.options.meta.type === 'tt';
}
