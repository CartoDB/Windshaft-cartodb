...
// this is meant as a hack for development (otherwise we'd be coupling
// this to Camshaft impolementation)
...
function overviews_query_with_zoom_expression(query, overviews, zoom_level_expression) {
    ...
    var replacement;
    _.each(_.keys(overviews), function(table) {
...
function overview_table_for_zoom_level(table_overviews, zoom_level) {
    ...
    if ( table_overviews ) {
        ...
        if ( !overview_table ) {
            _.every(_.keys(table_overviews).sort(function(x,y){ return x-y; }), function(overview_zoom) {
...
function apply_filters_to_query(query, filters) {
    // TODO: it'd be nice to have a public API in camshaft to apply filters!
    if ( filters && !_.isEmpty(filters)) {
        query = queryBuilder.getSql(query, filters);
    }
    return query;
}
...
OverviewsQueryRewriter.prototype.query = function(query, data, options) {
    ...
    var overviews        = data && data.overviews;
    var unfiltered_query = data && data.unfiltered_query;
    ...
    if ( !overviews || !this.is_supported_query(unfiltered_query)) {
        return query;
    }
    ...
    if ( rewritten_query === unfiltered_query ) {
        ...
        rewritten_query = apply_filters_to_query(rewritten_query, filters);
    }
