...
module.exports = class ApiRouter {
    ...
    route (app, routes) {
        ...
        routes.forEach(route => {
            ...
            if (preRoutingMiddlewares !== undefined && preRoutingMiddlewares.trim()) {
                preRoutingMiddlewares.split(',').forEach(middlewarePath => {
                    try {
                        ...
                        this.serverOptions.logger.error({ exception: e, path: middlewarePath }, 'Prerouting middleware not found, skipping');
                    }
                });
            }
            ...
            apiRouter.use(errorMiddleware({ logger: this.serverOptions.logger }));
