...
module.exports = class TurboCartoAdapter {
    getMapConfig (user, requestMapConfig, params, context, callback) {
        ...
        layers.forEach((layer, index) => {
            ...
            if (context.aggregation && context.aggregation.layers) {
                layerAggContext = context.aggregation.layers[index];
            }

            parseCartoQueue.defer(parseTurboCartoCss, params, layer, index, layerId, layerAggContext);
        });
...
function parseTurboCartoCss (userDbParams, layer, layerIndex, layerId, layerAggContext, callback) {
    ...
    getTokens(pg, layer, (err, tokens) => {
        ...
        const layerSql = getLayerQuery(layer, layerAggContext);
        const sql = SubstitutionTokens.replace(layerSql, tokens);
...
function getLayerQuery (layer, layerAggContext) {
    let layerQuery = layer.options.sql;

    if (layerAggContext.isDefaultAgg) {
        if (layer.options.sql_wrap_no_aggregated) {
            layerQuery = layer.options.sql_wrap_no_aggregated;
        } else {
            layerQuery = layer.options.sql_raw;
        }
    }
