...
var garbageCollectorStats = require('gc-stats');
...
module.exports = function(serverOptions) {
    ...
    var gc = garbageCollectorStats();

    gc.on('stats', function (stats) {
        global.statsClient.timing('windshaft.gc.time', stats.pauseMS);
        global.statsClient.gauge('windshaft.gc.type', stats.gctype);

        for (var propAfter in stats.after) {
            if (stats.after.hasOwnProperty(propAfter)) {
                global.statsClient.gauge('windshaft.gc.' + propAfter, stats.after[propAfter]);
            }
        }

        for (var propDiff in stats.diff) {
            if (stats.diff.hasOwnProperty(propDiff)) {
                global.statsClient.gauge('windshaft.gc.diff.' + propDiff, stats.diff[propDiff]);
            }
        }
    });
