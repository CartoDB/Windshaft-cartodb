...
OverviewsMetadataApi.prototype.getOverviewsMetadata = function (dbConfig, sql, callback) {
    ...
    this.pgQueryRunner.run(dbConfig, query, function handleOverviewsRows(err, rows) {
        if (err){
            callback(err);
            return;
        }
