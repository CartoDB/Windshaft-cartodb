...
module.exports = class AggregationMapConfigAdapter {
        ...
        return new Promise((resolve, reject) => {
            this._shouldAdaptLayer(connection, mapConfig, layer, index, (err, shouldAdapt) => {
                ...
                if (sqlQueryWrap) {
                    ...
                    layer.options.sql_wrap_no_aggregated = sqlQueryWrap.replace(/<%=\s*sql\s*%>/g, layer.options.sql);
                }
    ...
    _getAggregationMetadata (mapConfig, layer, index, adapted) {
        ...
        const isDefaultAgg = mapConfig._isDefaultLayerAggregation(index);

        if (!adapted) {
            return { isDefaultAgg: false, png: false, mvt: false};
        }

        if (mapConfig.isVectorOnlyMapConfig()) {
            return { isDefaultAgg, png: false, mvt: true };
        }

        return { isDefaultAgg, png: true, mvt: true };
    }
};
