...
function createOptionsForNamedMapConfigProvider(context, fullPrivileges = false) {
    const options = {};
    ...
    options.zoom = context.zoom;
    options.lon = context.lon;
    options.lat = context.lat;
...
function NamedMapsController(prepareContext, namedMapProviderCache, tileBackend, previewBackend,
    surrogateKeysCache, tablesExtentApi, metadataBackend, pgConnection, mapStore, templateMaps) {
...
NamedMapsController.prototype.register = function(app) {
    app.get(
        ...
        authMiddleware({ metadataBackend: this.metadataBackend, allowNamedMap: true }),
        userMiddleware.getMetadata({ metadataBackend: this.metadataBackend }),
        dbMiddleware({ fullPrivileges: true }),
    ...
    app.get(
        ...
        userMiddleware.getUsername(),
        authMiddleware({
            metadataBackend: this.metadataBackend,
            allowNamedMap: true,
            mapStore: this.mapStore,
            templateMaps: this.templateMaps
        }),
        userMiddleware.getMetadata({ metadataBackend: this.metadataBackend }),
        dbMiddleware(),
...
NamedMapsController.prototype.getNamedMapProvider = function (label) {
    return function getNamedMapProviderMiddleware (req, res, next) {
        ...
        const options = createOptionsForNamedMapConfigProvider(res.locals, true);

        this.namedMapProviderCache.get(user, template_id, config, auth_token, options, (err, namedMapProvider) => {
            if (err) {
                err.label = label;
                return next(err);
            }
...
NamedMapsController.prototype.prepareLayerFilterFromPreviewLayers = function (label) {
    return function prepareLayerFilterFromPreviewLayersMiddleware (req, res, next) {
        ...
        const options = createOptionsForNamedMapConfigProvider(res.locals);

        // overwrites 'all' default filter
        options.layer = layerVisibilityFilter.join(',');

        // recreates the provider
        this.namedMapProviderCache.get(user, template_id, config, auth_token, options, (err, provider) => {
            if (err) {
                err.label = label;
                return next(err);
            }
...
NamedMapsController.prototype.getStaticImageOptions = function () {
    return function getStaticImageOptionsMiddleware(req, res, next) {
        const { namedMapProvider, template } = res.locals;
        ...
        namedMapProvider.getAffectedTablesAndLastUpdatedTime((err, affectedTablesAndLastUpdate) => {
            ...
            const { db: dbConfig } = res.locals;
            const connection = this.pgConnection.getConnection(dbConfig);

            this.tablesExtentApi.getBounds(connection, affectedTables, (err, bounds) => {
                if (err) {
                    return next();
                }
