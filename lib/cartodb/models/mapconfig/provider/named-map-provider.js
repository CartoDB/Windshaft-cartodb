...
function createOptionsForWindshaft(context) {
...
NamedMapMapConfigProvider.prototype.getMapConfig = function(callback) {
    if (!!this.err || this.mapConfig !== null) {
        return callback(this.err, this.mapConfig, createOptionsForWindshaft(this.rendererParams), this.context);
    }
    ...
    step(
        ...
        function getUserApiKey(err) {
            assert.ifError(err);
            self.metadataBackend.getUserMapKey(self.owner, this); //TODO eneko use params.userContext
        },
        ...
        function prepareAdapterMapConfig(err, requestMapConfig) {
            assert.ifError(err);
            const dbConfig = self.params.db;
            context.analysisConfiguration = {
                user: self.owner,
                db: self.pgConnection.getMasterConnectionParams(dbConfig),
                batch: {
                    username: self.owner,
                    apiKey: apiKey
                }
        ...
        function cacheAndReturnMapConfig(err, renderLimits) {
            ...
            return callback(self.err, self.mapConfig, createOptionsForWindshaft(self.params), self.context);
        }
    );
};
