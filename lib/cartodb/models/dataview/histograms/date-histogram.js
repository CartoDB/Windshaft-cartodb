...
const dateOverrideBasicsQueryTpl = ctx => `
    __cdb_basics AS (
        SELECT
            max(${ctx._end})::float AS __cdb_max_val,
            min(${ctx._start})::float AS __cdb_min_val,
            avg(date_part('epoch', ${ctx._column})) AS __cdb_avg_val,
            min(
                date_trunc(
                    '${ctx._aggregation}',
...
module.exports = class DateHistogram extends BaseHistogram {
    ...
    _buildQueryTpl (ctx) {
        return `
            WITH
            ${this._hasOverridenRange(ctx._override) ? dateOverrideBasicsQueryTpl(ctx) : dateBasicsQueryTpl(ctx)},
