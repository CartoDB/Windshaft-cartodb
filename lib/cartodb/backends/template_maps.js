...
TemplateMaps.prototype.addTemplate = function(owner, template, callback) {
    ...
    self._redisCmd('HLEN', [ userTemplatesKey ], (err, numberOfTemplates) => {
        if (err) {
            return callback(err);
        }
        ...
        self._redisCmd('HSETNX', [ userTemplatesKey, templateName, JSON.stringify(template) ], (err, wasSet) => {
            if (err) {
                return callback(err);
            }

            if (!wasSet) {
                const ownerExistsError = new Error(`Template '${templateName}' of user '${owner}' already exists`);
                return callback(ownerExistsError);
            }
