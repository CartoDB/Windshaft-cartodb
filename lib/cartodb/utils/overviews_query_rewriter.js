...
function overviews_view_for_table(table, overviews_metadata, indent) {
    ...
    for (i = 0, len = sorted_overviews.length; i < len; i++) {
        ...
        overview_layers.push([overview_z_condition(z_lo, z_hi), ov_table, false]);
        z_lo = z_hi;
    }
    overview_layers.push(["_vovw_z > " + z_lo, table, true]);
    ...
    selects = overview_layers.map(function(condition_table) {
      ...
      var isBase = condition_table[2]; // true if it corresponds to the base table
      ov_table.schema = ov_table.schema || parsed_table.schema;
      var ov_identifier = TableNameParser.table_identifier(ov_table);
      if (isBase) {
        return indent + "SELECT *, 1 AS _vovw_count FROM " + ov_identifier + ", _vovw_scale WHERE " + condition;
      } else {
