...
function overviews_query(query, overviews, zoom_level_expression) {
    ...
    var replacement;
    var definite_zoom = zoom_level_expression && (''+zoom_level_expression).match(/^\d+$/);
    for ( var table in overviews ) {
        if (overviews.hasOwnProperty(table)) {
            ...
            if ( definite_zoom) {
                  var otable_info = table_overviews[''+zoom_level_expression];
                  if ( otable_info ) {
                    replacement = otable_info.table; // TODO: support ranges...
                  }
            } else {
...
OverviewsQueryRewriter.prototype.query = function(query, data, z) {
    ...
    return overviews_query(query, overviews, +z);
};
