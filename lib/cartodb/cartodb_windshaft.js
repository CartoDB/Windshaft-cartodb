...
var CartodbWindshaft = function(serverOptions) {
   var debug = global.environment.debug;
    ...
    // Routes used to create maps with GET,
    // for which we don't want to request any caching.
    // POST requests are never cached
    var mapCreateRoutesGET = [
    ...
    ws.sendResponse = function(res, args) {
      ...
      Step (
        function addCacheChannel() {
          ...
          if ( _.contains(mapCreateRoutesGET, req.route.path) ) {
            // We do not want to cache
            // map creation responses
