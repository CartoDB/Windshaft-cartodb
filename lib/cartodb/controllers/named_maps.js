...
function NamedMapsController(app, mapStore, templateMaps, metadataBackend, mapBackend, tileBackend, previewBackend,
                             templateBaseUrl, surrogateKeysCache, layergroupRequestDecorator, tablesExtentApi) {
    this.app = app;
...
NamedMapsController.prototype.tile = function(req, res) {
    ...
    step(
        ...
        function getImage(err) {
            ...
            self.tileBackend.getTile(new MapStoreMapConfigProvider(self.mapStore, req.params), params, this);
        },
...
NamedMapsController.prototype.staticMap = function(req, res) {
    ...
    step(
        ...
        function getImage(err, imageOpts) {
            ...
            var provider = new MapStoreMapConfigProvider(self.mapStore, imageParams);
            if (!_.isUndefined(imageOpts.zoom) && imageOpts.center) {
                self.previewBackend.getImage(
                    provider, imageParams, width, height, imageOpts.zoom, imageOpts.center, this);
            } else {
                self.previewBackend.getImage(
                    provider, imageParams, width, height, imageOpts.bounds, this);
            }
        },
