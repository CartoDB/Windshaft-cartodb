...
const aggregateColumnNames = (ctx, table) => {
    ...
    if (table) {
        return sep(Object.keys(columns).map(
            column_name => `${sqlId(table)}.${sqlId(column_name)}`
        ));
    }
    return sep(Object.keys(columns).map(column_name => sqlId(column_name)));
};
...
const dimensionNames = (ctx, table) => {
    ...
    if (table) {
        return sep(Object.keys(dimensions).map(
            dimension_name => `${sqlId(table)}.${sqlId(dimension_name)}`
        ));
    }
    return sep(Object.keys(dimensions).map(column_name => sqlId(column_name)));
};
...
const groupingExpression = (expr) => {
    // TODO: restrict expr to column names, apply sqlId to it
    return expr;
};
