...
function parseTurboCartoCss (userDbParams, layer, layerIndex, layerId, layerAggContext, callback) {
    ...
    getTokens(pg, layer, (err, tokens) => {
        ...
        let layerSql = layer.options.sql;

        // TODO: move it to aggregation mapconfig adapter
        if (layerAggContext.isDefaultAgg) {
            if (layer.options.sql_wrap) {
                layerSql = layer.options.sql_wrap.replace(/<%=\s*sql\s*%>/g, layer.options.sql_raw);
            } else {
                layerSql = layer.options.sql_raw;
            }
        }
