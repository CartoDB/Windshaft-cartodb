...
describe('Authorize', () => {
  ...
  describe('Authorization granted', () => {
    ...
    it('Save API key to context', (next) => {
      ...
      authorize({}, res, (err) => {
        expect(res.locals.auth.apiKey).to.exist;
        next();
      });
    });
  });
  ...
  describe('Authorization denied', () => {
    ...
    it('API key username VS user mismatch', (next) => {
      ...
      authorize({}, res, (err) => {
        ...
        expect(err.message).to.equal('api-key-username-mismatch');
        next();
      });
    });
    ...
    it('No apikey', (next) => {
      ...
      authorize({}, res, (err) => {
        expect(err.status).to.equal(401);
        expect(err.message).to.equal('api-key-required');
        next();
      })
    });
    ...
    it('API key does not grant access to API', (next) => {
      ...
      authorize({}, res, (err) => {
        ...
        expect(err.message).to.equal('api-key-doesnt-grant-access-to-this-api');
        next();
      })
    });
