...
function getDbMasterParams(context) {
    return {
        host: context.db.host,
        name: context.db.name,
        user: context.db.masterUser,
        password: context.db.masterPassword,
...
MapController.prototype.getAffectedTables = function () {
    return function getAffectedTablesMiddleware (req, res, next) {
        const { layergroup, mapconfig } = res.locals;

        const dbParams = getDbMasterParams(res.locals);

        this.pgConnection.getConnection(dbParams, (err, connection) => {
            if (err) {
                return next(err);
            }
            ...
            QueryTables.getAffectedTablesFromQuery(connection, sql.join(';'), (err, affectedTables) => {
                ...
                this.layergroupAffectedTables.set(dbParams.name, layergroup.layergroupId, affectedTables);
...
MapController.prototype.setLayerStats = function () {
    return function setLayerStatsMiddleware(req, res, next) {
        const { mapconfig, layergroup } = res.locals;

        this.pgConnection.getConnection(getDbMasterParams(res.locals), (err, connection) => {
            if (err) {
                return next(err);
            }
