...
module.exports = ({metadataBackend, apiName}) => {
  ...
  return (req, res, next) => {
    ...
    fetchApiKeyFromMetadataBackend({metadataBackend, username, apiKeyToken, apiName}, (err, apiKey) => {
      if ( err || ! apiKey) {
        return next(createError(401, 'api-key-not-found'), {type: 'auth', subtype: 'api-key-not-found', context: {apiKeyToken}});
      }

      if ( ! apiKeyGrantsAccessToThisApi(apiKey)) {
        return next(createError(403, 'api-key-doesnt-grant-access-to-this-api'), {apiKey}, {type: 'auth', subtype: 'api-key-doesnt-grant-access-to-this-api'});
      }
