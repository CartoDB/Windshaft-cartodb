    ...
    if (ttName) {
        ...
        var filters = relatedDataviewKeys.reduce(function(filters, relatedDataviewKey) {
            if (requestFilters.dataviews.hasOwnProperty(relatedDataviewKey)) {
                ...
                relatedFilter.column_type = getColumnType(datasource, relatedFilter.column);
...
function getColumnType(datasource, column) {
    if (datasource.node.schema.hasOwnProperty(column)) {
        return datasource.node.schema[column];
    }
}
...
/**
 * CREATE OR REPLACE FUNCTION TT_Formula(
 *  tt_table TEXT, bbox_json JSON, filters_json JSON[], parameters_json JSON, zoom_level INT DEFAULT NULL
 * )
 */
...
function mercator2webmercatorBbox(bboxFilter) {
    return (bboxFilter === null) ? null : mercator.convert(bboxFilter.bboxes[0], '900913').join(',');
}

TTDatasource.getDataviewQuery = function(definition, dataview, dataviewName, bboxFilter, overrideParams, ownFilter) {
    var ttMetadata = definition.source.meta;
    var bbox = mercator2webmercatorBbox(bboxFilter);

    function dataviewNameFilter(filter) {
        return (ownFilter) ? true : filter.dataview !== dataviewName;
    }

    return dataviewTemplateFn(dataview)({
        table: ttMetadata.table,
        bbox: bbox,
        filters: ttMetadata.filters.filter(dataviewNameFilter).map(filterParams).map(pgJson).join(','),
        parameters: JSON.stringify(dataviewParameters(dataview, overrideParams))
    });
};

function dataviewTemplateFn(dataview) {
    switch (dataview.getType()) {
        case 'histogram':
            return histogramQueryTemplate;
        case 'aggregation':
            return aggQueryTemplate;
        case 'formula':
            return formulaQueryTemplate;
        default:
            return () => null;
    }
}

function dataviewParameters(dataview, overrideParams) {
    switch (dataview.getType()) {
        case 'histogram':
            return histogramParameters(dataview, overrideParams);
        case 'aggregation':
            return aggregationParameters(dataview);
        case 'formula':
            return formulaParameters(dataview);
        default:
            return {};
    }
}

function histogramParameters(dataview, overrideParams) {
    return {
        column: dataview.column,
        column_type: dataview._columnType,
        bins: overrideParams.bins,
        start: overrideParams.start,
        end: overrideParams.end,
        aggregation: overrideParams.aggregation || dataview.aggregation || 'auto'
    };
}

function aggregationParameters(dataview) {
    return {
        column: dataview.column,
...
function formulaParameters(dataview) {
    return {
