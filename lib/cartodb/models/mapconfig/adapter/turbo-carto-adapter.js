...
TurboCartoAdapter.prototype._parseCartoCss = function (username, params, layer, layerIndex, layerId, callback) {
    ...
    const layerRawSql = layer.options.sql_raw;

    if (hasWrappedQuery(layer)) {
        return getTokens(pg, layerRawSql, (err, tokens) => {
            if (err) {
                return callback(err);
            }
 ...
 */
function getTokens (pg, sql, callback) {
    var tokensQuery = tokensQueryTpl({ _sql: sql });
